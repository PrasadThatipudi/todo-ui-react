...
const Task = (props) => {
  const { done, description, priority, toggleTask, deleteTask, isFocused } =
    props;
  ...
  const renderPriorityBadges = (priority) => {
    ...
    const badges = [];
    if (priority & 1) {
      // Check if important (priority includes 1)
      badges.push(
    ...
    if (priority & 2) {
      // Check if urgent (priority includes 2)
      badges.push(
  ...
  return (
    <div
      className={`task ${isFocused ? "task-focused" : ""}`}
      onClick={() => toggleTask()}
    >
      <span className={`task-desc ${done ? "done" : "undone"}`}>
        {description}
      </span>
      <div className="task-priority">{renderPriorityBadges(priority)}</div>
